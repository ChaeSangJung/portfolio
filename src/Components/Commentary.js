...
const Commentary = ({type}) => {
    ...
    return (
        <Container>
          ...
          : type === "cms" ? (
            <TextComment>
              App과 web의 콘텐츠를 관리할 수 있는 CMS입니다. 전체 퍼블리싱을 하였고 퍼블리싱 후에 backend 개발자가 java,spring을 이용하여 만들었습니다.<br/>
              ...
              방송관리 : 라이브 방송을 관리하는 페이지입니다. 방송 상태 중에서 예정, 진행을 클릭하고 들어가면 채팅화면과 방송화면 프롬프터가 나옵니다.<br/>
              상품관리 : 등록되어 있는 상품들을 관리할 수 있는 페이지입니다.<br/>
              회원관리 : 회원을 관리할 수 있는 페이지이며 새창을 통해 회원 구분을 변경할 수 있습니다.<br/>
              지난주문관리 : 5월 이전의 지난 주문을 관리할 수 있는 페이지이며, 주문 상태를 변경할 수 있습니다.<br/>
              이슈 : 크로스 브라우징<br/>
          ...
          : type === "webview" ? (
            <TextComment>
              ...
              협의, 회의는 길거나 짧다고 해서 좋은 것이 아니라 상대방의 입장을 고려하기가 중요하다고 많이 느꼈습니다.<br/>
          ...
          : type === "web-project" ? (
            <TextComment>
              제가 맡은 부분은 장바구니와 마이 페이지입니다.<br/>
              장바구니는 상품을 담아 놓을 수 있고, 수량을 변경할 수 있으며, 장바구니에서 삭제를 할 수 있습니다.<br/>
              마이페이지는 말 그대로 나의 정보를 담고 있는 페이지입니다.<br/>              
              전체를 다 해보지는 않았지만 react/nextjs 조합으로 처음 만들어 보는 커머스 프로젝트였습니다.<br/>
              처음 해보는 작업이라 전체 flow를 이해하는 데 애를 먹었지만 기획자, 디자이너와 이야기해가며 커머스 페이지의 전체 flow를 이해해가며 작업을 했습니다.
            </TextComment>
          )
          : type === "homepage" ? (
            <TextComment>
